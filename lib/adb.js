...
module.exports = {
  ...
  async getSource() {
    let out = await this.shell('uiautomator', ['dump']);
  ...
  querySelector(selector, tree) {
    let filter = function (element) {
      return Selector.isMatch(element, selector);
    }
    let nodes = this.findElementsByFilter(tree, filter);
...
async function test() {
  ...
  // ====================================================
  let tree = await adb.getDocument();
  let nodes = adb.querySelector('[text="支付成功"],[text="完成"],#com.alipay.android.app:id/rl_nav_bar,~收款方, .android.widget.ImageView', tree);
  console.log(nodes.map(node => node.attributes));
  // ====================================================
  ...
  await adb.clickByPos(250, 400);

  await adb.wait(2000);

  ele = await adb.getElementByAttribute('content-desc', '确认付款');
  adb.click(ele);

  await adb.wait(2000);

  ele = await adb.getElementByAttribute('text', '立即付款');
  adb.click(ele);
  ...
  ele = await adb.getElementByAttribute('text', '完成');
  adb.click(ele);

  // adb.clickByPos(250, 400)
  let t1 = Date.now();
